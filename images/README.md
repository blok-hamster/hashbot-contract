...
`sample/` stops at the SVG, because that is where the art ends. `tokenURI` has
two more layers on top of it, and they nest:

    raw PNG bytes
      -> base64                        -> data:image/png;base64,...
        -> six of those inside one SVG
          -> base64                    -> data:image/svg+xml;base64,...
            -> that string as the "image" field of a JSON object
              -> base64                -> data:application/json;base64,...

That last string is what `tokenURI` returns. **Base64 nothing until the last
possible moment.** Base64 costs a third more bytes, so encoding early and
carrying the result through the next layer pays the tax twice. Storage holds
raw bytes; the first `base64` should happen in memory inside the `view` call.

The metadata schema is yours &mdash; `name`, `description`, `attributes` from
`names.json`, whatever else you want. The only field this folder constrains is
`image`, and only in that it must be the data URI above and not a URL.

Measured on one bot, so the shape of the bill is visible before you build it:

    storage actually read     ~11,700 bytes   (12 records, 5 chunks)
    PNG constants inlined         348 bytes   (contract code, not storage)
    SVG produced               ~16,700 bytes
    tokenURI produced          ~30,300 bytes

All of that after the first row is computed per call, in a `view` function, so
nobody pays gas for it. It matters only in that some RPC providers cap response
size; ~30 KB is comfortable.
...
- **A metadata schema.** `names.json` has the strings and *The outer wrap*
  specifies the `image` field; the rest of the JSON is your call.
- **Solidity for the renderer.** The steps are specified byte-for-byte above
  and `sample/` is the fixture to test against. If you want a reference
  implementation to diff behaviour against, ask &mdash; there is a working one in
  plain JavaScript (`scripts/prove-onchain.mjs`) that builds a bot from the
  chunk files and nothing else.
